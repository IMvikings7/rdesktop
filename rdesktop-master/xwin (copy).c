...
extern Pixmap* g_tempPixmap = 0; // itbeckham
extern Pixmap* g_correctedPixmap = 0;
extern uint8* g_tempPixmapBuf = 0;
uint32 g_gamma_limit;
double g_gamma_value;
...
ui_create_window(void)
{
	...
	printf("g_tempPixmap created: %d, %d", g_width, g_height);
	//g_tempPixmapBuf = (uint8*)malloc(g_width * g_height * g_depth / 8);
	//g_tempPixmap = XCreateBitmapFromData(g_display, g_wnd, g_tempPixmapBuf, g_width, g_height);
	g_tempPixmap = XCreatePixmap(g_display, g_wnd, g_width, g_height, g_depth);     // itbeckham
...
ui_paint_bitmap(int x, int y, int cx, int cy, int width, int height, uint8 * data)
{
	...
	tdata = (g_owncolmap ? data : translate_image(width, height, data));
	//memset(tdata, 0x80, width * height * bitmap_pad / 8); // itbeckham

	buflen = width * height * bitmap_pad / 8; // itbeckham
	printf("::::::::::::::::::::::::: ui_paint_bitmap -- bitmap_pad: %d, gamma_value: %f\n", bitmap_pad, g_gamma_value);
	for(i = 0; i < buflen; i++){
		tdata[i] = (uint8)(tdata[i] * g_gamma_value);
	}

	image = XCreateImage(g_display, g_visual, g_depth, ZPixmap, 0,
			     (char *) tdata, width, height, bitmap_pad, 0);
	XPutImage(g_display, g_tempPixmap, g_gc, image, 0, 0, x, y, cx, cy);
...
ui_create_glyph(int width, int height, uint8 * data)
{
	...
	//memset(data, 0x80, width * height); // itbeckham
...
XImage* getCorrectedImage(){    // itbeckham
	...
	printf(",,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,  XImage; %d, %d, %d, %d\n", img->width, img->height, img->depth, img->bitmap_pad);
	...
	for(i = 0; i < buflen; i+=bpp){
		//energy += pixel_energy((unsigned char)img->data[i + 2], (unsigned char)img->data[i + 1], (unsigned char)img->data[i]);
	...
	printf(",,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,  energy: %d, average energy: %d, gamma_limit: %d\n", energy, avenergy, g_gamma_limit);
	...
	for(i = 0; i < buflen; i+=bpp){
//		img2->data[i] = (char)(int)((double)img->data[i] * rate);
...
*/
	...
	//XCopyPlane(g_display, (Pixmap) src, g_wnd, g_gc, srcx, srcy, cx, cy, x, y, 1);
	//XCopyArea(g_display, (Pixmap) src, g_wnd, g_gc, srcx, srcy, cx, cy, x, y);
	XCopyArea(g_display, (Pixmap) src, g_tempPixmap, g_gc, srcx, srcy, cx, cy, x, y);

	XImage* img = getCorrectedImage();

	if(img == 0){
		XCopyArea(g_display, g_tempPixmap, g_wnd, g_gc, 0, 0, g_width, g_height, 0, 0);
	}else{
...
//	XCopyArea(g_display, g_tempPixmap, g_wnd, g_gc, 0, 0, g_width, g_height, 0, 0);
	...
	ON_ALL_SEAMLESS_WINDOWS(XCopyArea,
				(g_display, (Pixmap) src, sw->wnd, g_gc,
				 srcx, srcy, cx, cy, x - sw->xoffset, y - sw->yoffset));
	if (g_ownbackstore){
		printf("--------------- ui_memblt g_ownbackstore\n");
		XCopyArea(g_display, (Pixmap) src, g_backstore, g_gc, srcx, srcy, cx, cy, x, y);
	}
	RESET_FUNCTION(opcode);
